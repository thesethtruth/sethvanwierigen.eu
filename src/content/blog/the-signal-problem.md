...
The discourse around AI capability is almost entirely about execution: what the model can do, how fast it codes, whether it can pass a bar exam. This framing is wrong. Execution does matter but this narrative mistakes one layer for the whole stack. The real question isn't how capable AI is. It's how legible the domain is.

## The stack

Think of this as a mental model for keeping your bearings. Most debate collapses everything into "capability", but AI transformation in any domain operates through multiple layers, not execution alone. The stack below is a diagnostic: it helps you name which layer is actually moving, and which one is still the bottleneck. When it feels like AI suddenly "clicks" for a domain, it’s usually because one layer stopped being the constraint. Keeping the layers separate makes it easier to see what actually changed, and helps explain why software moved first while other domains won’t follow the same curve.
...
So most domains will remain messy for a while. But the pockets where signal is already clean enough will be unrecognizable in two years. The hard part is that both of those things are true at the same time.


## Use the stack as a mental model

The stack is a way to keep your grip on what’s happening. It keeps you from mistaking a jump in execution for a universal capability leap. When something looks transformative, ask what actually changed: cheaper substrate, tighter execution, clearer direction, stronger origination, or better signals tying the layers together. Software is the extreme case where all of these line up. Most domains won’t be. Disruption will follow the gradient of legibility and value. The human edge will concentrate where intent is disputed and what-to-build still has to be originated.