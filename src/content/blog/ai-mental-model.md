...
A model does not act on the world by itself. AI capability is a system property: a model embedded in a substrate, wrapped in tools, and connected to feedback, evaluators, and human decision-making. When AI “clicks” in a domain it’s rarely because the model suddenly became a genius.
...
2. **Direction** is where intent meets decision. It's interpretation — reading between lines, reconciling contradictions, navigating the gap between what someone says and what will actually serve them. It's also disciplining the solution space: opinionated tooling and conventions constrain how the system searches. And it's shot-calling: choosing between structurally different valid paths. AI is getting better at interpretation, but decision-making is fundamentally harder — the signal is social, contextual, and often deliberately ambiguous. 
3. **Origination** is having the vision in the first place. Not interpreting what someone else wants, but seeing what's missing. As AI compresses the layers below it, this is where human leverage concentrates — but not how people usually mean. When execution is cheap and direction disciplined, vision becomes partially procedural. Lots of people can generate ideas; the scarce thing is selection: picking the right one, committing under uncertainty, and making it stick against institutional inertia. 
4. 
And then there's the **environment** the system operates in. Two constraints cut across the stack. First is **verifiability**: whether the domain has enforceable evaluators, contracts, and tests that can reliably tell you when you're wrong. Second is **accountability**: whether anyone is allowed to delegate action, and who bears the downside when the loop converges to the wrong thing. A domain can be legible and still resist transformation if correctness can’t be checked or responsibility can’t be assigned.
...
This is also where the surrounding institutional environment matters, in a very specific way: it determines what can be instrumented, what evaluators can be enforced, and what delegation is even allowed.
...
Software is a special case at every layer of the stack, not just substrate and execution. Direction is unusually disciplined. Even poorly voiced intent — "let the user log in" — resolves almost automatically because the model has seen ten thousand login flows and because the solution is channeled by existing frameworks, conventions, and libraries.
...
Some domains resist for a different reason that looks similar from the outside: modeling debt. It’s not only that the solution space is wide — it’s that the causal structure is uncertain, feedback is delayed, and outcomes are confounded. Even clean signals don’t make iteration cheap if reality won’t yield fast, attributable feedback.

## Signal quality

AI's penetration into any domain is a function of signal infrastructure, not model capability alone. Think about a construction project. The signal passes through multiple lossy human interfaces before anyone can act on it. Even if the AI is brilliant at each node, the loop is degraded by the interfaces between them. This reframes the entire AI transformation question. It's not "how smart is the model." It's "how legible is the domain."
...
We don’t merely accelerate the tasks we already do. We change what is worth attempting. When the cost of a try collapses and the stack compresses, you start exploring ideas that used to be irrational because failure was too expensive, coordination too painful, feedback too slow. Many people can produce plausible options now. The edge goes to people and organisastions who can pick what matters, and commit through uncertainty and the cost of change. 
